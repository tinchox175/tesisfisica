...
for j in fil:
    ...
    # if (j.split('_')[3]).split('.')[0] in ['280', '160', '85']:
        ...
        continue
    if (j.split('_')[3]).split('.')[0] == '291':
        vm = -10
        vx = 2
    elif (j.split('_')[3]).split('.')[0] == '150':
        vm = -10
        vx = 2
    elif (j.split('_')[3]).split('.')[0] == '90':
        vm = -125
        vx = 10
    elif (j.split('_')[3]).split('.')[0] == '50':
        vm = -400
        vx = 400
    elif (j.split('_')[3]).split('.')[0] == '11':
        vm = -2000
        vx = 3000
    X = []
    Y = []
    ...
    yi = np.linspace(np.min(Y0), np.max(Y0), 500)
    Xi, Yi = np.meshgrid(xi, yi)
    Zi = griddata((X0, Y0), Z0, (Xi, Yi), method='linear')
    cemap = LinearSegmentedColormap.from_list("custom_cmap", list(colors))
    mesh = ax.pcolormesh(Xi, Yi, Zi, antialiased=True, shading='gouraud', cmap=cemap, norm=norm)
...
for j in fil:
    ...
    for i in files:
        ...
        Y0 = np.append(Y0,Y)
        Z0 = np.append(Z0,Z)
        norm = Normalize(vmin=-700, vmax=10)
        positions, color_names = zip(*colors)
        ax.axvline(26,0,200, c='red', ls='--', alpha=0)
        ax.axvline(180,0,200, c='gray', ls='--', alpha=1)
        ax.axvline(29000, 0, 200, c='gray', ls='--', alpha=0)
    ...
    cemap = LinearSegmentedColormap.from_list("custom_cmap", list(colors))
    mesh = ax.pcolormesh(Xi, Yi, Zi, antialiased=True, shading='gouraud', cmap=cemap, norm=norm)
    contour = ax.contour(Xi, Yi, Zi, alpha=1, levels=[-0.2], colors='Grey', linestyle='dashed', linewidths=4)
    contour = ax.contourf(Xi, Yi, Zi, alpha=0.7, levels=[-9999,-0.2], cmap='Greys', antialiased=True, linestyle='dashed', linewidths=10)
    plt.colorbar(mesh, label='Z\' ($\Omega$)')
    plt.tight_layout()
    # break
