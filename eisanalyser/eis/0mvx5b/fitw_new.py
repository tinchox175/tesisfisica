...
t = ['290', '270', '250', '230', '210', '190', '170', '150', '130', '110', '90', '70']
initial_guess = [5, 0.46, 22e-9, -1.47, 47e-8, 5 , 1e-6]
for i in t:
    data = np.genfromtxt(f'e:/porno/tesis 3/tesisfisica/eis/0mvx5b/{i}k0.00mV_eis', unpack=True, delimiter='', skip_header=1)
    f = data[2][1:-1]
    Z = data[0][1:-1] - 1j*data[1][1:-1]
    if i == '130':
        initial_guess = [250, 46.7, 1.11e-8, -250, 10e-15, 2 , 1e-2]
    elif i == '110':
        # break
        initial_guess = [250, 46.7, 1.11e-8, -250, 10e-15, 93 , 7.5]
    elif i == '90':
        initial_guess = [256, 46.7, 1.11e-8, -254, 10e-15, 40 , 10]
    elif i == '70':
        f = data[2][1:-3]
...
for i in np.flip(files[3:]):
    ...
    sc = ax[0].scatter(re, -im, s=4, label=f'T = {T}K', color=color)
    # Ensure the second argument is the same size as w
    z_vals = z(w, *(param[n] for param in params_tuple))
    ax[0].plot(z_vals.real, -z_vals.imag, color=color, alpha=0.7, zorder=3)
    ax[1].scatter(w, np.sqrt(re**2+im**2), s=4, color=color)
    ax[1].plot(w, np.sqrt(z_vals.real**2+z_vals.imag**2), color=color, alpha=0.7, zorder=3)
    ax[2].scatter(w, np.arctan(im/re), color=color, s=4)
    ax[2].plot(w, np.arctan(z_vals.imag/z_vals.real), color=color, alpha=0.7, zorder=3)
    handles.append(sc)
...
ax[0].set_ylim(-10,15)
ax[1].set_xlabel(' ')
ax[1].set_ylabel('|Z| (Ohm)')
ax[1].set_xscale('log')
ax[1].set_yscale('log')
# ax[1].set_ylim(1.9e0,4e0)
