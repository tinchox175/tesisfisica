...
for i in t:
    ...
    plt.show()
    print(circuit)
data = np.genfromtxt('C:/tesisfisica/IVs/Parametros_ajustados.csv', unpack=True, delimiter=',', skip_header=1)
print(data)
#%%
from impedance import preprocessing
from impedance.models.circuits import CustomCircuit
...
for i in t:
    ...
    plt.show()
    print(circuit)
data = np.genfromtxt('C:/tesisfisica/IVs/Parametros_ajustados_b.csv', unpack=True, delimiter=',', skip_header=1)
print(data)
#%%
data = np.genfromtxt('C:/tesisfisica/IVs/Parametros_ajustados_b.csv', unpack=True, delimiter=',', skip_header=1)
...
from impedance.models.circuits import CustomCircuit
import csv
with open('Parametros_ajustados_c.csv', mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['T', 'L1', 'R2', 'C2', 'R3', 'R1'])
t = ['280', '260', '240', '220', '200', '180', '160', '140', '120', '100', '85']
initial_guess = [0.855,3.48,24.9e-9,-20,-20]
for i in t:
    data = np.genfromtxt(f'C:/tesisfisica/eis/0mvx5a/{i}k0.00mV_eis', unpack=True, delimiter='', skip_header=1)
    f = data[2][1:]
    Z = data[0][1:] - 1j*data[1][1:]

    circuit = 'p(L1,R2,C2,R1)-R3'
    circuit = CustomCircuit(circuit, initial_guess=initial_guess)
    circuit.fit(f, Z, 
                bounds=([0, 0, 0, -330, -330],
                        [np.inf, np.inf, 10, 0, 0]))

    paramteres = circuit.parameters_
    initial_guess = circuit.parameters_
    with open('Parametros_ajustados_c.csv', mode='a', newline='') as file:
        writer = csv.writer(file)
        writer.writerow([i] + list(paramteres))
    print(f'T = {i}K')
    circuit.plot(f_data=f, Z_data=Z, kind='nyquist')
    ...
    plt.show()
    print(circuit)
data = np.genfromtxt('C:/tesisfisica/IVs/Parametros_ajustados_c.csv', unpack=True, delimiter=',', skip_header=1)
print(data)
#%%
data = np.genfromtxt('C:/tesisfisica/eis/0mvx5a/Parametros_ajustados_c.csv', unpack=True, delimiter=',', skip_header=1)
T, Lr, Rl, Cl, Rn = data
fig, ax= plt.subplots(figsize=(10, 6))
fig, ax2 = plt.subplots(figsize=(10, 6))
ax2.set_ylabel('Capacitancia (F)')
...
ax.plot(1/T, Rl, 'o-', label='Rl', c='#e07b67')      # Rl
ax2.plot(1/T, Cl, 'x-', label='Cl', c='#a1e067')     # Cl
ax.plot(1/T, Rn, 'x-', label='Rn', c='#67e0e0')     # Rn
lines, labels = ax.get_legend_handles_labels()
lines2, labels2 = ax2.get_legend_handles_labels()
ax.legend(lines + lines2, labels + labels2, loc='center', bbox_to_anchor=(0.7, 0.5))
ax.grid()
