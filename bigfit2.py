...
for i in files:
    ...
    R4 = np.array(R4)
    R4e = np.array(R4e)
    ax2.errorbar(1/T, A, yerr=Ae, marker='o', markersize=9, label='A', color='#F1B74A')
    ax.errorbar(1/T, R, yerr=Re, marker='o', markersize=9, label='R', color='#4A85F1')
    line1 = ax2.scatter(-1,-1, marker='_', label='A', lw=4, color='#F1B74A')
    line2 = ax.scatter(-1,-1, marker='_', label='R', lw=4, color='#4A85F1')
    # line3 = ax.errorbar(1/T, R4, yerr=R4e, marker='o', markersize=9, label='Log($R_{4T}$)', color='#E63946')
    ax.set_xlabel('1/T (1/K)')
    ax.set_yscale('log')
    ax2.set_yscale('log')
    ax.set_xlim(0.003, 0.015)
    # ax.set_ylim(20,200)
    # ax2.set_ylim(0.65*1e-3,4*1e-3)
    # ax2.set_xlim(310, 85)
    ax.set_ylabel('R ($\Omega$)')
    ax2.set_ylabel('A ($A/V^2$)')
    lines = [line1, line2]
...
for i in files:
    ...
    # R4 = np.array(R4)
    # R4e = np.array(R4e)
    line1 = ax2.errorbar(1/T, A, marker='x', markersize=9, label='$A$', color='#F1B74A')
    line2 = ax.errorbar(1/T, R, yerr=Re, marker='x', markersize=9, label='$R$', color='#4A85F1')
...
for i in files:
    ...
    # R4 = np.array(R4)
    # R4e = np.array(R4e)
    line1 = ax2.errorbar(1/T, A, yerr=Ae, marker='^', markersize=9, label='$A$', color='#F1B74A')
    line2 = ax.errorbar(1/T, R, yerr=Re, marker='^', markersize=9, label='$R$', color='#4A85F1')
    # ax.set_xlabel('1/T (1/K)')
    # ax.set_yscale('log')
    # ax2.set_yscale('log')
    # ax.set_ylim(2e1,2e2)
...
for i in files:
    ...
    R4e = np.array(R4e)
    if n==0:
        line1 = ax2.errorbar(1/T, A, yerr=Ae, marker='o', markersize=9, label='$A$', color='#F1B74A')
        line2 = ax.errorbar(1/T, R, yerr=Re, marker='o', markersize=9, label='$R$', color='#4A85F1')
        # line3 = ax.errorbar(1/T, np.log(R4), yerr=R4e/R4, marker='o', markersize=9, label='Log($R_{4t}$)', color='#E63946')
    else:
        line1 = ax2.errorbar(1/T, A, yerr=Ae, marker='none', linestyle='dashed', markersize=9, label='$A$', color='#F1B74A')
        line2 = ax.errorbar(1/T, R, yerr=Re, marker='none', linestyle='dashed', markersize=9, label='$R$', color='#4A85F1')
        # line3 = ax.errorbar(1/T, np.log(R4), yerr=R4e/R4, marker='none', linestyle='dashed', markersize=9, label='Log($R_{4t}$)', color='#E63946')
    line1 = ax2.scatter(-1,-1, marker='_', label='A', color='#F1B74A')
    line2 = ax.scatter(-1,-1, marker='_', label='R', color='#4A85F1')
    n+=1
    ax.set_xlabel('1/T (1/K)')
    ax.set_yscale('log')
    ax2.set_yscale('log')
    ax.set_xlim(0.003, 0.021)
    ax.set_ylim(10,1e4)
    ax2.set_ylim(8*1e-1*1e-3,1e1*1e-3)
    # ax2.set_xlim(310, 85)
    ax.set_ylabel('R ($\Omega$)')
    ax.set_xticks([0.005, 0.010, 0.015, 0.02])
    ax2.set_ylabel('A ($A/V^2$)')
ax.grid()
lines = [line1, line2]
labels = [line.get_label() for line in lines]
