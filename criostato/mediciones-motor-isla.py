...
config_ls = np.genfromtxt('./configlsisla.txt', delimiter=',')
...
global instrument_v
instrument_v = a34420a.Agilent34420A("GPIB0::7::INSTR")
global instrument_c
...
with dpg.window(label="Archivo", width=w, height=130, pos=(0,0)):
    ...
    dpg.add_text(default_value=f"{os.getcwd()}", tag="selected_file_text")
...
def add_row(values, ctrl=0):
    ...
    try:
        ...
        with open("./Archivos/test" + str(time.strftime("%H %M %S", time.localtime())), mode='a', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(values)
...
def lecturas(mode = 'bip'): #esta funcion lee los aparatos y guarda todo lo que lee en el registro final
    ...
    if data_r[0]>1e20:
        data_r = N_stat_msr()
    data_ls = data_r[2]
    dpg.set_value("tl", f'{np.round(data_ls[0],2)} K')
    dpg.set_value("sl", f'{np.round(data_ls[1],2)} K')
    dpg.set_value("pl", f'{np.round(data_ls[2],0)} %')
    ...
    dpg.set_value("r_1_p", [t_plot[:-1], r1_plot[:-1]])
    dpg.set_value("r_2_p", [t_plot[:-1], r2_plot[:-1]])
    dpg.set_value("r_1_p_latest", [t_plot[-1:], r1_plot[-1:]])
    dpg.set_value("r_2_p_latest", [t_plot[-1:], r2_plot[-1:]])
    dpg.set_value("R1_p", [T_plot[:-1], r1_plot[:-1]])
    dpg.set_value("R2_p", [T_plot[:-1], r2_plot[:-1]])
    dpg.set_value("R1_p_latest", [T_plot[-1:], r1_plot[-1:]])
    dpg.set_value("R2_p_latest", [T_plot[-1:], r2_plot[-1:]])
...
with dpg.window(label="Fuente & Nanovoltímetro", width=w, height=160, pos=(0,130)):
    dpg.add_text("V-Lim k224", pos=(20,20))
    ...
    dpg.add_text("I k224", pos=(120, 20))
    ...
    dpg.add_text("Max. V 34420", pos=(20, 100))
    v_scale_max = dpg.add_input_text(label="V", default_value=f"{0.01}", tag="v_scale_max", width=60, pos=(20,120))
    dpg.add_text("Mín. V 34420", pos=(120, 100))
    v_scale_min = dpg.add_input_text(label="V", default_value=f"{0.001}", tag="v_scale_min", width=60, pos=(120,120))
    dpg.add_text("Rango V 34420", pos=(220, 100))
    rvolt = dpg.add_input_text(default_value=f"{0.1}", width=40, tag="v_range", pos=(220, 120))

with dpg.window(label="Control Temperatura", width=w, height=60, pos=(0,290)):
    ...
    dpg.add_text("Voltaje motor", pos=(150, 30))
    ...
    dpg.add_text("Motor Máx", pos=(270, 20))
    motorl = dpg.add_input_text(default_value=f"{1.2}", width=40, tag="mlo", pos=(270, 80))
    dpg.add_text("Motor Mín", pos=(270, 60))
...
def medicion_T(sender, app_data, user_data):
    ...
    with dpg.window(label="Control en vivo", width=w, height=240, pos=(0,390)):
        ...
        dpg.add_text("Tiempo total", pos=(10,140)) #ttl
        ttal_actual_display = dpg.add_text("00:00:00", tag="ttl", pos=(10,160))
        dpg.add_text("Tiempo tarea", pos=(180,140))#ttal
