...
def lecturas(): #esta funcion lee los aparatos y guarda todo lo que lee en el registro final
    ...
    dpg.set_value("tl", data_ls[0]+' K')
    dpg.set_value("sl", data_ls[1]+' K')
    dpg.set_value("pl", data_ls[2]+' %')
    switch.open_channels([211,212,221,222])
    switch.close_channels([211,221])
    data_rs = []
    data_r = N_stat_msr()
    dpg.set_value("r1l", np.round(data_r[0]/data_r[1],3)+' Ohm')
    r1_plot.append(data_r[0]/data_r[1])
    r1_plot.append(data_r[0])
    ...
    controller = LakeShore340(gpib_address=12)
    fuente = LakeShore625(11)
    dpg.set_value("r2l", np.round(data_r[0]/data_r[1],3)+' Ohm')
    r2_plot.append(data_r[0]/data_r[1])
    data_rs.append(data_r[0])
    data_rs.append(data_r[1])
    data_rs.append(data_r[0]/data_r[1])
    data_b = [float(fuente.get_current()), float(fuente.get_voltage()), 1.1914*float(fuente.get_current())]
    dpg.set_value("if", data_b[0],' A')
    dpg.set_value("vb", data_b[1],' A')
    dpg.set_value("hel", data_b[2],' G')
    ...
    dpg.set_value("I_p", [t_plot, Ib_plot])
    dpg.fit_axis_data('t_temp_ax_c')
    dpg.fit_axis_data('Curr_y')
    dpg.set_value("H_p", [t_plot, H_plot])
    dpg.fit_axis_data('t_temp_ax_c')
    dpg.fit_axis_data('H_y')
...
with dpg.window(label="Keithley 224", width=w, height=160, pos=(0,130)):
    ...
    dpg.add_text("Max. V (muestra)", pos=(20, 100))
    v_scale_max = dpg.add_input_text(label="V", default_value=f"{0.01}", tag="v_scale_max", width=60, pos=(20,120))
    dpg.add_text("Mín. V (muestra)", pos=(150, 100))
    v_scale_min = dpg.add_input_text(label="V", default_value=f"{0.001}", tag="v_scale_min", width=60, pos=(150,120))
...
with dpg.window(label="LakeShore 340", width=w, height=140, pos=(0,290)):
    ...
    dpg.add_text("Tiempo estable (s)", pos=(20,60))
    heater_display_text = dpg.add_input_text(default_value=f"{dpg.get_value("HR_m")}", width=40, tag="HR_in", pos=(160, 80))
    dpg.add_text("Heater Range", pos=(160, 60))
    dpg.add_button(label='Update LS340', callback=lambda:update_vars(), pos=(20,100))
...
def medicion_T(sender, app_data, user_data):
    ...
    with dpg.window(label="Control en vivo", width=w, height=260, pos=(0,390)):
        ...
        dpg.add_text("Campo est. (G)", pos=(235,60))
...
def update_vars(): #esta funcion lee los parametros puestos en la ventana y los actualiza en vivo con el multithread
    ...
    controller.close()
    np.savetxt(directory+'configls.txt', [f'{dpg.get_value("P_in")}, {dpg.get_value("I_in")}, {dpg.get_value("D_in")}, {dpg.get_value("HR_in")}'], fmt='%s')
...
def ramp_H(I_actual, I):
    ...
    dpg.set_value('tal', "Subiendo a ultima corriente sin switch")
...
def ramp_H_sin_PS(I):
    ...
    fuente.limit(60, 3, 0.17) #algunos limites a nivel software
    fuente.write('PSHS 1 55 10') #parametros del switch
    fuente.set_voltage(2.8) #por las dudas vuelvo a poner el limite de Cryo
    fuente.set_ramp_rate(0.15) #le pongo la velocidad a la fuente
    fuente.psh(1) #prendo el switch persistente
    dpg.set_value('tal', "Esperando conexión del switch")
    while fuente.get_status_switch() != 1.0:
...
def medir_tabla_T_en_H():#a temperaturas fijas barre campos
    update_vars()
    ...
    for fila in algo_T: #en cada temperatura corro el protocolo estabilizar y barrer campos
        ...
        for filah in algo_H: #acá va a barrer campos para cada temperatura dada
            ...
            update_vars()
            ramp_H_sin_PS(setpoint_H/1191.4)
...
def medir_tabla_H_en_T():#a campos fijos barre temperatura
    update_vars()
    tiempo_start = time.time() #pongo tiemp os iniciales y levanto las listas que se van a medir
    tiempo_est = 0
    final_T = 0
    final_H = 0
    controller = LakeShore340(gpib_address=12)
    ...
    algo_T = dpg.get_item_children("Tabla", 1)
    algo_H = dpg.get_item_children("Tabla_H", 1)
    dpg.set_value('I_actual', 0)
    for filah in algo_H:
        parametros_H = dpg.get_item_children(filah, 1)
        setpoint_H = dpg.get_value(parametros_H[0])
        update_vars()
        ramp_H(float(dpg.get_value('I_actual')), setpoint_H/1191.4)
        for fila in algo_T: #en cada temperatura corro el protocolo estabilizar y barrer campos
            parametros_T = dpg.get_item_children(fila, 1)
            update_vars() #estos updates sirven por si se actualizan parametros del PID, heater o medición
            setpoint = dpg.get_value(parametros_T[0])
            ...
            while True: #una vez que se va abajo del threshold intenta estabilizar por x segundos
                ...
                if tiempo_est < float(dpg.get_value("T_est")):
                    ...
                    break
            for i in np.arange(1,int(dpg.get_value("ctd_msr"))): #hago muchas lecturas en cada temperatura, no se si es necesario
                lecturas()
        final_H = setpoint_H
    ramp_H(final_H, 0) #vuelvo el campo a 0 con el mismo protocolo
    controller.write('CSET 1, A, 1, 0')
...
with dpg.window(label="Medición T", width=370, height=650, pos=(w,0)):
    ...
    with dpg.group(tag="g_col_H", pos=(10,460)):
        with dpg.table(header_row=True, tag="Tabla_H", borders_innerH=True, borders_outerH=True,
                        borders_innerV=True, borders_outerV=True, height = 150, no_host_extendY = True,
                        scrollY = True):
            dpg.add_table_column(label="H (G)")
            dpg.add_table_column(label="Medir?")
    dpg.add_button(label="Limpiar", callback=reset_table_H, pos=(20,620))
    dpg.add_button(label="Comenzar (H fijo)", callback=medicion_H, pos=(120,620))
    dpg.add_button(label="Comenzar (T fijo)", callback=medicion_T, pos=(240,620))
...
with dpg.window(label='Campo', pos=(w+370+275,0)):
    with dpg.plot(height=250, width=260):
            dpg.add_plot_axis(dpg.mvXAxis, label="Tiempo (s)", tag="t_temp_ax_c")
            dpg.add_plot_axis(dpg.mvYAxis, label="Corriente (A)", tag="Curr_y")
            dpg.add_line_series([], [], label="Corriente", parent="t_temp_ax_c", tag="I_p")
            dpg.add_plot_axis(dpg.mvYAxis, label="Campo (G)", tag="H_y")
            dpg.add_line_series([], [], label="Campo", parent="t_temp_ax_c", tag="H_p")

with dpg.window(label='Campo', pos=(w+370+275,285)):
    with dpg.plot(height=250, width=260):
            ...
            dpg.add_line_series([], [], label="Resistencia 1", parent="t_temp_ax_r", tag="R1_p")
            dpg.add_line_series([], [], label="Resistencia 2", parent="t_temp_ax_r", tag="R2_p")
